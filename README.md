...
* `opts`: Configuration options for the autocannon instance. This can have the following attributes. _REQUIRED_.
    ...
    * `har`: the `String` path on disk of an [HAR](https://w3c.github.io/web-performance/specs/HAR/Overview.html) file. Autocannon will extract request from this file, keeping only the ones targeting the same domains as the given urls. Requests method, path, query parameters, headers and body will be set from the HAR file. _NOTE_: this will take over `requests` property. You can add extra headers with `headers` property, but `method`, `form` and `body` will be ignored. _OPTIONAL_
