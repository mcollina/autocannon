...
function Client (opts) {
  ...
  this.parser[HTTPParser.kOnMessageComplete] = () => {
    ...
    if (!this.destroyed) {
      ...
      this.requestIterator.recordBody(resp.req, resp.headers.statusCode, resp.body, toHeaderKeyValue(resp.headers.headers || []))
  ...
  function toHeaderKeyValue (headersArray) {
    const rv = {}

    for (let i = 0; i < headersArray.length - 1; ++i) {
      rv[headersArray[i]] = headersArray[i + 1]
    }
    return rv
  }
