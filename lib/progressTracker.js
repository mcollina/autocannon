...
function track (instance, opts) {
  ...
  if (opts.renderProgressBar && opts.outputStream.isTTY) {
    if (!iOpts.amount) { // duration progress bar
      instance.on('tick', curriedTick(durationProgressBar))
      instance.on('done', curriedTick(durationProgressBar, iOpts.duration - 1))
      process.once('SIGINT', curriedTick(durationProgressBar, iOpts.duration - 1))
    } else { // amount progress bar
      instance.on('response', curriedTick(amountProgressBar))
      instance.on('reqError', curriedTick(amountProgressBar))
      instance.on('done', curriedTick(amountProgressBar, iOpts.amount - 1))
      process.once('SIGINT', curriedTick(amountProgressBar, iOpts.amount - 1))
    }
  }
...
function curriedTick (progressBar, amount) {
  return () => { progressBar.tick(amount) }
}
