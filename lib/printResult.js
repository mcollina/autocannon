...
const printResult = (result, opts) => {
  ...
  logToStreamOrLocalStr(shortLatency.toString())
  ...
  logToStreamOrLocalStr(requests.toString())
  logToStreamOrLocalStr('')
  logToStreamOrLocalStr('Req/Bytes counts sampled once per second.\n')
  ...
  if (opts.renderLatencyTable) {
    ...
    logToStreamOrLocalStr(latencies.toString())
    logToStreamOrLocalStr('')
  }

  if (result.non2xx) {
    logToStreamOrLocalStr(`${result['2xx']} 2xx responses, ${result.non2xx} non 2xx responses`)
  }
  logToStreamOrLocalStr(`${format(result.requests.total)} requests in ${result.duration}s, ${prettyBytes(result.throughput.total)} read`)
  if (result.errors) {
    logToStreamOrLocalStr(`${format(result.errors)} errors (${format(result.timeouts)} timeouts)`)
  }
  if (result.mismatches) {
    logToStreamOrLocalStr(`${format(result.mismatches)} requests with mismatched body`)
  }
  if (result.resets) {
    logToStreamOrLocalStr(`request pipeline was resetted ${format(result.resets)} ${result.resets === 1 ? 'time' : 'times'}`)
  }

  function logToStreamOrLocalStr (msg) {
    if (opts.returnAsString) {
      strResult += msg + '\n'
    } else {
