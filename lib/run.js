...
function run (opts, cb) {
  ...
  url.responseMax = typeof amount === 'number'
                    ? (amount / opts.connections) : 0 || opts.maxConnectionRequests
  ...
  for (let i = 0; i < opts.connections; i++) {
    if (amount && typeof amount === 'number') {
      url.responseMax = (Math.floor(amount / opts.connections) + (((i + 1) <= (amount % opts.connections)) ? 1 : 0))
    }
