...
const defaultOptions = {
  ...
  pipelining: 1,
  timeout: 10,
  maxConnectionRequests: false,
  maxOverallRequests: false,
  requests: [{}]
}
...
function run (opts, cb) {
  ...
  if (opts.maxOverallRequests && opts.maxOverallRequests < 1 && opts.maxOverallRequests < opts.connections) {
    cb(new Error('maxOverallRequests can not be < 1 or < opts.connections'))
    return
  }
  ...
  if (!url.responseMax) {
    url.responseMax = typeof opts.maxOverallRequests === 'number'
                      ? (opts.maxOverallRequests / opts.connections) : false
    // this needs work if ((maxOverallRequests % opts.connections) > 0)
  }
