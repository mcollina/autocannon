...
function validateOpts (opts) {
  if (opts.numWorkers) {
    if (opts.setupClient) {
      console.error('Can not use setupClient option while using workers')
      process.exit(1)
    }

    if (opts.requests && opts.requests.some(r => r.setupRequest || r.onResponse)) {
      console.error('Can not use setupRequest/onResponse options while using workers')
      process.exit(1)
    }
  }
}
...
function run (opts, cb) {
  ...
  const tracker = _run(opts, cb)
  tracker.on('done', (result) => {
    handleFinish(result, opts)
  })
