...
const { parseHARSync } = require('./parseHAR')
...
function _run (opts, cb, tracker) {
  ...
  const optsRequests = opts.har ? parseHARSync(opts.har) : opts.requests
  ...
  const urls = ofURL(opts.url, true).map(url => {
    ...
    if (Array.isArray(optsRequests)) {
      url.requests = new Array(optsRequests.length)
      let j = 0
      for (const request of optsRequests) {
        // when request have an origin (typically, parsed from HAR files), only keep the ones to our url
        if (!request.origin || request.origin === origin) {
          url.requests[j++] = request
        }
      }
      url.requests.splice(j)
    }
